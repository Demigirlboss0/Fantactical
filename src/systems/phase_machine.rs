...
use crate::model::maneuver_legality::{
    available_maneuvers, combo_whitelist, is_directed_offensive,
};
use crate::model::rolls::{check_roll, crit_hit_table, roll_3d6, roll_damage, RollOutcome};
use crate::model::{
    ActorId, CritHitResult, ExtraEffort, HitLocation, ManeuverPayload, ManeuverRelation,
    ManeuverType, Posture, RelationState, TurnPhase,
};
    ...
    match phase {
        TurnPhase::ManeuverSelection => {
            for ev in maneuver_events.read() {
                ...
                if ev.source_id != cur.current_actor {
                    info!(
                        "PhaseMachine: skipping — source {} != current {}",
                        ev.source_id, cur.current_actor
                    );
                    continue;
                }
                let Some(actor) = cur.actors.get(&ev.source_id) else {
                    continue;
                };
                    ...
                    if ev.maneuver == ManeuverType::Move {
                        if let Some(hex) = ev.target_hex {
                            ...
                            if dist <= max_dist && dist > 0 {
                                ...
                                info!(
                                    "Move: {} can't reach {:?} (dist={}, max={})",
                                    a.name, hex, dist, max_dist
                                );
                            }
                        }
                    }
                ...
                let is_aim_or_eval =
                    matches!(ev.maneuver, ManeuverType::Aim | ManeuverType::Evaluate);
                ...
                if let Some(target_id) = ev.target_id {
                    let payload = if ev.maneuver == ManeuverType::Aim {
                        let weapon_acc = actor.attacks.first().and_then(|a| a.acc).unwrap_or(0);
                        ManeuverPayload::AimBonus {
                            accumulated: 0,
                            turns: 0,
                            weapon_acc,
                        }
                ...
                let attacks_per_turn = new_state
                    .actors
                    .get(&ev.source_id)
                    .unwrap()
                    .attacks_per_turn;
                ...
                if offensive {
                    ...
                    info!(
                        "PhaseMachine: offensive {:?} → AttackSetup, attacks_remaining={}",
                        ev.maneuver, attacks_per_turn
                    );
                ...
                log_events.send(
                    LogEvent::info(format!("{} declared {:?}", name, ev.maneuver))
        ...
        TurnPhase::AttackSetup => {
            ...
            for ev in attack_events.read() {
                let cur = history.current();
                let Some(attacker) = cur.actors.get(&ev.attacker_id) else {
                    continue;
                };
                let Some(attack) = attacker.attacks.get(ev.attack_index) else {
                    continue;
                };
                ...
                match attacker.posture {
                    Posture::Prone => {
                        mods.push(("Prone posture".into(), -4));
                        total_mod -= 4;
                    }
                    Posture::Kneeling | Posture::Crawling => {
                        mods.push(("Kneeling/Crawling".into(), -2));
                        total_mod -= 2;
                    }
                    _ => {}
                }
                ...
                if let Some(target) = cur.actors.get(&ev.target_id) {
                    ...
                    match target.posture {
                        Posture::Prone => {
                            mods.push(("Target prone".into(), -3));
                            total_mod -= 3;
                        }
                        Posture::Kneeling | Posture::Crawling | Posture::Sitting => {
                            mods.push((format!("Target {:?}", target.posture), -2));
                            total_mod -= 2;
                        }
                        _ => {}
                    }
                ...
                for m in &cur.global_modifiers {
                    if matches!(
                        m.applies_to,
                        crate::model::ModifierTarget::AllRolls
                            | crate::model::ModifierTarget::AttackRolls
                    ) {
                ...
                new_state
                    .actors
                    .get_mut(&ev.attacker_id)
                    .unwrap()
                    .active_attack = Some(ev.attack_index);
        ...
        TurnPhase::AttackRoll => {
            ...
            for ev in roll_events.read() {
                ...
                log_events.send(
                    LogEvent::info(format!(
                        "Attack roll: {} vs {}, outcome {:?}",
                        ev.roll, skill, outcome
                    ))
                    .with_phase_context(ctx_round, ctx_turn, phase),
                );
                ...
                match outcome {
                        ...
                        if let Some(idx) = a.active_attack {
                            if let Some(atk) = a.attacks.get(idx) {
                                ...
                                log_events.send(
                                    LogEvent::info(format!(
                                        "Damage roll: {}d{:+} = {} (crit: {:?})",
                                        atk.damage_dice, atk.damage_adds, base_dmg, crit
                                    ))
                                    .with_phase_context(ctx_round, ctx_turn, phase),
                                );
                                modal.last_outcome_text.push(format!(
                                    "Damage: {}d{:+} = {}",
                                    atk.damage_dice, atk.damage_adds, base_dmg
                                ));
                            }
                        }
                    ...
                    RollOutcome::Success => {
                        modal.pending_crit_result = None;
                        modal.last_outcome_text =
                            vec![format!("Roll: {} — HIT (vs {})", ev.roll, skill)];
                        let a = cur.actors.get(&cur.current_actor).unwrap();
                        if let Some(idx) = a.active_attack {
                            if let Some(atk) = a.attacks.get(idx) {
                                ...
                                log_events.send(
                                    LogEvent::info(format!(
                                        "Damage roll: {}d{:+} = {}",
                                        atk.damage_dice, atk.damage_adds, dmg
                                    ))
                                    .with_phase_context(ctx_round, ctx_turn, phase),
                                );
                                modal.last_outcome_text.push(format!(
                    ...
                    _ => {
                        modal.pending_crit_result = None;
                        modal.last_outcome_text =
                            vec![format!("Roll: {} — MISS (vs {})", ev.roll, skill)];
                        modal.show = false;
                        new_state.current_phase = TurnPhase::Complete;
                    }
                }
            ...
            for ev in defense_events.read() {
                ...
                match outcome {
                    RollOutcome::CriticalSuccess | RollOutcome::Success => {
                        modal
                            .last_outcome_text
                            .push(format!("Defense: {} vs {} — DEFENDED!", roll, d_val));
                        modal.show = false;
                        new_state.current_phase = TurnPhase::Complete;
                    }
                    _ => {
                        modal
                            .last_outcome_text
                            .push(format!("Defense: {} vs {} — FAILED!", roll, d_val));
                    ...
                    if relation.source == cur.current_actor {
                        match &mut relation.payload {
                            ManeuverPayload::AimBonus {
                                ...
                                *turns += 1;
                                *accumulated = (*weapon_acc + *turns - 1).min(*weapon_acc + 2);
                            }
                            ManeuverPayload::EvaluateBonus { accumulated } => {
...
fn advance_turn(
    ...
    new_state.relations.retain(|r| {
        matches!(
            r.payload,
            ManeuverPayload::AimBonus { .. } | ManeuverPayload::EvaluateBonus { .. }
        )
    });
    ...
    if has_extra_attacks {
        ...
        new_state.attacks_remaining = new_state
            .actors
...
fn apply_pending_injury(
    ...
    let Some(d_id) = modal.target_id_for_modal else {
        return;
    };
        ...
        if let Some(idx) = attacker.active_attack {
            if let Some(atk) = attacker.attacks.get(idx) {
                ...
                let dmg = if modal.rolled_damage > 0 {
                    ...
                    log_events.send(
                        LogEvent::info(format!(
                            "Damage roll (re-roll): {}d{:+} = {}",
                            atk.damage_dice, atk.damage_adds, re_rolled
                        ))
                        .with_context(ctx_round, ctx_turn),
                    );
                    re_rolled
                };
                match resolve_injury(cur, d_id, loc, dmg, atk.damage_type, crit, shock_enabled) {
                    Ok((inj_state, o)) => {
                        *new_state = inj_state;
                        modal.last_outcome_text.push(format!(
                            "Injury: {} {:?} to {:?}, DR {} → {} hp",
                        ...
                        if o.knockdown {
                            modal
                                .last_outcome_text
                                .push(format!("Knockdown! (mod {})", o.knockdown_modifier));
                        }
                        if o.dead {
...
fn denies_active_defenses(maneuver: Option<ManeuverType>) -> bool {
    matches!(
        ...
        Some(ManeuverType::AllOutAttackDetermined)
            ...
            | Some(ManeuverType::AllOutConcentrate)
    )
}
...
fn compute_defense_options(state: &crate::model::GameState, modal: &mut ModalState) {
    ...
    let Some(target) = state.actors.get(&tid) else {
        return;
    };
    ...
    modal
        .defense_options
        .push((DefenseType::Dodge, target.dodge()));
